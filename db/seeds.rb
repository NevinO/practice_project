...
titles = %w(personal work family fun).map {|titles| Title.create! name: title }
puts 'Creating formats'
formats = %w(personal work family fun).map {|formats| Format.create! name: format }
puts 'Creating tags'
...
puts 'Removing comments for the last post, so at least one post is uncommented'
Post.last.comments.each do |comment|
  comment.guest.destroy
  comment.destroy
end
